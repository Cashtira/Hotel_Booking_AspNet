...
public class BookingService(IBookingRepository bookingRepository, IMapper mapper) : IBookingService
{
    ...
    {
        var bookings = await this.bookingRepository.GetAllBookingsAsync().ConfigureAwait(true);
        return this.mapper.Map<List<BookingDTO>>(bookings);
    }

    public async Task<BookingDTO?> GetBookingByIdAsync(int bookingId)
    {
        var booking = await this.bookingRepository.GetBookingByIdAsync(bookingId).ConfigureAwait(true);
    ...
    public async Task AddBookingAsync(BookingDTO bookingDto)
    {
        ...
        await this.bookingRepository.AddBookingAsync(booking).ConfigureAwait(true);
    }

    public async Task UpdateBookingAsync(BookingDTO bookingDto)
    {
        var booking = this.mapper.Map<Booking>(bookingDto);
        await this.bookingRepository.UpdateBookingAsync(booking).ConfigureAwait(true);
    }

    public async Task DeleteBookingByIdAsync(int bookingId)
    {
        await this.bookingRepository.DeleteBookingByIdAsync(bookingId).ConfigureAwait(true);
    }
}
