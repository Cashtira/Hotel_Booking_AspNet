...
public class LoyaltyProgramRepository(ApplicationDbContext dbContext) : ILoyaltyProgramRepository
{
    ...
    {
        return await this.dbContext.LoyaltyPrograms.ToListAsync().ConfigureAwait(true);
    }

    public async Task<LoyaltyProgram?> GetLoyaltyProgramByIdAsync(int loyaltyProgramId)
    {
        return await this.dbContext.LoyaltyPrograms.FirstOrDefaultAsync(e => e.LoyaltyProgramId == loyaltyProgramId).ConfigureAwait(true);
    }
    ...
    {
        var loyaltyProgram = await this.GetLoyaltyProgramByIdAsync(loyaltyProgramId).ConfigureAwait(true);
