...
public class MaintenanceRepository(ApplicationDbContext dbContext) : IMaintenanceRepository
{
    ...
    {
        return await this.dbContext.Maintenances.ToListAsync().ConfigureAwait(true);
    }

    public async Task<Maintenance?> GetMaintenanceByIdAsync(int maintenanceId)
    {
        return await this.dbContext.Maintenances.FirstOrDefaultAsync(e => e.MaintenanceId == maintenanceId).ConfigureAwait(true);
    }
    ...
    {
        var maintenance = await this.GetMaintenanceByIdAsync(maintenanceId).ConfigureAwait(true);
        if (maintenance != null)
        {
