...
public class RoomTypeRepository(ApplicationDbContext dbContext) : IRoomTypeRepository
{
    ...
    {
        return await this.dbContext.RoomTypes.ToListAsync().ConfigureAwait(true);
    }

    public async Task<RoomType?> GetRoomTypeByIdAsync(int roomTypeId)
    {
        return await this.dbContext.RoomTypes.FirstOrDefaultAsync(e => e.RoomTypeId == roomTypeId).ConfigureAwait(true);
    }
    ...
    {
        var roomType = await this.GetRoomTypeByIdAsync(roomTypeId).ConfigureAwait(true);
        if (roomType != null)
        {
