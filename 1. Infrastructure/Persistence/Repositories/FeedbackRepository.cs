...
public class FeedbackRepository(ApplicationDbContext dbContext) : IFeedbackRepository
{
    ...
    {
        return await this.dbContext.Feedbacks.ToListAsync().ConfigureAwait(true);
    }

    public async Task<Feedback?> GetFeedbackByIdAsync(int feedbackId)
    {
        return await this.dbContext.Feedbacks.FirstOrDefaultAsync(e => e.FeedbackId == feedbackId).ConfigureAwait(true);
    }
    ...
    {
        var feedback = await this.GetFeedbackByIdAsync(feedbackId).ConfigureAwait(true);
        if (feedback != null)
        {
